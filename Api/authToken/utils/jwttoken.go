...
//验证token
func VerifyAction(token string) (*types.JWTClaims, error) {
	//解析token
	tokenString, err := jwt.ParseWithClaims(token, &types.JWTClaims{}, func(token *jwt.Token) (interface{}, error) {
	...
	//转换token里的信息
	claims, ok := tokenString.Claims.(*types.JWTClaims)
	if !ok {
